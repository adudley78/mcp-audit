...
~~**Layer 2 (Sigstore signature verification) and Layer 3 (dependency SBOM) are not yet implemented.**~~ **Layer 2 resolved 2026-04-23.** `scan --verify-signatures` fetches SLSA provenance bundles from the npm and PyPI attestation APIs and verifies them with the `sigstore` Python library (Fulcio cert chain, SCT, Rekor inclusion proof). OIDC subject from the signing certificate is compared against `RegistryEntry.repo` in the registry. Finding IDs ATTEST-010 through ATTEST-015 cover valid-match, valid-mismatch, invalid, expected-absent, absent, and error outcomes. Free for all tiers. `--strict-signatures` raises absent findings to MEDIUM. **Layer 3 (dependency SBOM analysis)** is still not yet implemented. Layer 1 covers hash pinning of the top-level package tarball; Layer 2 covers provenance attestation. Neither inspects transitive dependencies. See `docs/supply-chain.md` for the roadmap.
