...
class TransportAnalyzer(BaseAnalyzer):
    ...
    def analyze(self, server: ServerConfig) -> list[Finding]:
        ...
        if server.url:
            ...
            if parsed.scheme == "http" and parsed.hostname not in ("localhost", "127.0.0.1", "::1"):
                findings.append(Finding(
                    ...
                    description="SSE/HTTP server uses http:// to a non-localhost address",
        ...
        if server.transport == TransportType.STDIO and server.command:
            if server.command in ("sudo", "doas") or server.command.startswith("/usr/sbin"):
                findings.append(Finding(
                    id="TRANSPORT-002",
        ...
        if server.command in ("npx", "uvx", "bunx"):
            findings.append(Finding(
                ...
                description=f"Server uses {server.command} which downloads packages at runtime",
                evidence=f"Command: {server.command} {' '.join(server.args[:3])}",
                remediation="Install packages locally and reference the installed binary instead",
                cwe="CWE-829",
            ))
